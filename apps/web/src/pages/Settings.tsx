...
function getDefaultAlertAt() {
  ...
  const local = new Date(
    inOneHour.getTime() - inOneHour.getTimezoneOffset() * 60000,
  );
...
export function Settings() {
  ...
  const [calendarMode, setCalendarMode] =
    useState<GoogleCalendarMode>("single");
  ...
  const loadSettingsData = useCallback(async () => {
    ...
    try {
      ...
      if (teamData.currentTeamId) {
        const current = teamData.teams.find(
          (team) => team.id === teamData.currentTeamId,
        );
        if (current?.name) {
      ...
      if (
        ...
        !window.localStorage.getItem("expiry-alert.preferredTeamId")
      ) {
        window.localStorage.setItem(
          "expiry-alert.preferredTeamId",
          String(teamData.currentTeamId),
        );
      }
    } catch (error) {
      console.error(error);
  ...
  const handleForgotTeamPassword = async () => {
    ...
    try {
      ...
      console.error(error);
      showToast(
        error.message || t("settings.teamPasswordResetRequestFailed"),
        "error",
      );
    } finally {
      setTeamBusy(false);
  ...
  const handleCreateGoogleCalendarEvents = async () => {
    ...
    try {
      ...
      showToast(
        t("settings.googleCalendarEventsCreated", { count: result.created }),
        "success",
      );
    } catch (error: any) {
      console.error(error);
    ...
    <div className="container mx-auto p-6 max-w-4xl space-y-8">
      ...
      <div className="bg-card rounded-xl border p-6 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          {t("settings.teamWorkspaceDescription")}
        </p>
        <p className="text-xs text-muted-foreground">
          {t("settings.teamAutoAssignHelp")}
        </p>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={
                    teamBusy ||
                    joinTeamName.trim().length === 0 ||
                    joinTeamPassword.trim().length < 6
                  }
                >
                  {t("settings.joinTeam")}
                </Button>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            {t("settings.forgotTeamPasswordHelp")}
          </p>
        </div>
      </div>
