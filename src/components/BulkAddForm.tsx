    ...
    <div className="space-y-4 p-6 bg-muted/30 border rounded-lg">
      ...
      <div className="space-y-3">
        {reagents.map((reagent, index) => (
          <div
            key={index}
            className="grid grid-cols-1 md:grid-cols-6 gap-3 p-4 bg-background border rounded-lg"
          >
            <div>
              <Input
            ...
            <div>
              <Input
                type="date"
                placeholder={t('form.receivedDate')}
                value={reagent.receivedDate}
                onChange={(e) => updateReagent(index, 'receivedDate', e.target.value)}
              />
            </div>
            <div>
              <Input
